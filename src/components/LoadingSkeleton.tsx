export function PlayerCardSkeleton() {
  return (
    <div className="bg-gradient-to-br from-fdp-surface-1 to-fdp-bg-0 rounded-lg p-4 border border-fdp-border-2 card-enter hover-lift">
      <div className="flex items-center gap-3 mb-3">
        <div className="w-12 h-12 skeleton rounded-full ring-2 ring-fdp-border-2"></div>
        <div className="flex-1">
          <div className="h-4 skeleton rounded w-3/4 mb-2"></div>
          <div className="h-3 skeleton rounded w-1/2"></div>
        </div>
      </div>
      <div className="space-y-2">
        <div className="h-3 skeleton rounded w-full"></div>
        <div className="h-3 skeleton rounded w-5/6"></div>
        <div className="h-3 skeleton rounded w-4/6"></div>
      </div>
    </div>
  );
}

export function TradeCardSkeleton() {
  return (
    <div className="bg-fdp-surface-1 rounded-lg p-5 border border-fdp-border-2 card-enter">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 skeleton rounded-full"></div>
          <div>
            <div className="h-5 skeleton rounded w-32 mb-2"></div>
            <div className="h-3 skeleton rounded w-24"></div>
          </div>
        </div>
      </div>
      <div className="grid md:grid-cols-3 gap-4 mb-4">
        <div className="bg-fdp-bg-0 rounded-lg p-4 border border-fdp-border-2">
          <div className="h-3 skeleton rounded w-20 mb-2"></div>
          <div className="h-6 skeleton rounded w-16"></div>
        </div>
        <div className="bg-fdp-bg-0 rounded-lg p-4 border border-fdp-border-2">
          <div className="h-3 skeleton rounded w-20 mb-2"></div>
          <div className="h-6 skeleton rounded w-16"></div>
        </div>
        <div className="bg-fdp-bg-0 rounded-lg p-4 border border-fdp-border-2">
          <div className="h-3 skeleton rounded w-20 mb-2"></div>
          <div className="h-6 skeleton rounded w-16"></div>
        </div>
      </div>
    </div>
  );
}

export function ListSkeleton({ count = 3 }: { count?: number }) {
  return (
    <div className="space-y-3">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-fdp-surface-1 rounded-lg p-4 border border-fdp-border-2 card-enter" style={{ animationDelay: `${i * 0.1}s` }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 skeleton rounded-full"></div>
            <div className="flex-1">
              <div className="h-4 skeleton rounded w-3/4 mb-2"></div>
              <div className="h-3 skeleton rounded w-1/2"></div>
            </div>
          </div>
        </div>
      ))}
    </div>
  );
}

export function StatCardSkeleton() {
  return (
    <div className="bg-gradient-to-br from-fdp-bg-0 to-fdp-surface-1 rounded-lg border border-fdp-border-2 p-6 card-enter">
      <div className="h-4 skeleton rounded w-32 mb-3"></div>
      <div className="h-8 skeleton rounded w-20 mb-2"></div>
      <div className="h-3 skeleton rounded w-24"></div>
    </div>
  );
}

export function TableSkeleton({ rows = 5, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="space-y-2">
      <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
        {Array.from({ length: cols }).map((_, i) => (
          <div key={`header-${i}`} className="h-4 skeleton rounded"></div>
        ))}
      </div>
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div
          key={`row-${rowIndex}`}
          className="grid gap-4 py-3 border-t border-fdp-border-2 card-enter"
          style={{ gridTemplateColumns: `repeat(${cols}, 1fr)`, animationDelay: `${rowIndex * 0.05}s` }}
        >
          {Array.from({ length: cols }).map((_, colIndex) => (
            <div key={`cell-${rowIndex}-${colIndex}`} className="h-3 skeleton rounded"></div>
          ))}
        </div>
      ))}
    </div>
  );
}

export function DashboardCardSkeleton() {
  return (
    <div className="bg-gradient-to-br from-fdp-bg-0 to-fdp-surface-1 rounded-lg border border-fdp-border-2 p-6 card-enter">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="h-4 skeleton rounded w-40 mb-2"></div>
          <div className="h-3 skeleton rounded w-24"></div>
        </div>
        <div className="w-10 h-10 skeleton rounded-lg"></div>
      </div>
      <div className="h-10 skeleton rounded w-32 mb-4"></div>
      <div className="flex items-center gap-2">
        <div className="h-3 skeleton rounded w-16"></div>
        <div className="h-3 skeleton rounded w-20"></div>
      </div>
    </div>
  );
}
