...
export default function KTCWRRankings() {
  ...
  const fetchWRValues = async () => {
    try {
      ...
      const mapped: WRValue[] = (data || []).map((p: any) => ({
    ...
    <div className="space-y-6">
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <PlayerAvatar
                        playerId={sleeperIdFor(wr.player_name, wr.player_id)}
                        espnId={getEspnIdFromCache(sleeperIdFor(wr.player_name, wr.player_id))}
                      ...
                      />
                      <div>
                        <div className="font-semibold text-fdp-text-1">{wr.full_name}</div>
                        <div className="text-xs text-fdp-text-3">Wide Receiver</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <span className="text-lg font-bold text-fdp-text-1">{wr.fdp_value.toLocaleString()}</span>
                    </div>
                  </td>
                </tr>
              )})}
      ...
      {filteredWrs.length === 0 && (
        <div className="bg-fdp-surface-1 border border-fdp-border-1 rounded-lg p-8 text-center">
          <p className="text-fdp-text-2">No wide receivers found matching your criteria</p>
        </div>
      )}
    </div>
  );
