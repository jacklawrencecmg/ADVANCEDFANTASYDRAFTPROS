...
export default function NotificationsPanel({ userId, leagueId }: NotificationsPanelProps) {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      default: return 'bg-fdp-text-3/20 border-fdp-text-3/30 text-fdp-text-3';
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {notifications.length === 0 ? (
          <div className="text-center py-12 bg-fdp-surface-1/50 rounded-lg border border-fdp-border-2">
            <Bell className="w-16 h-16 text-fdp-text-2 mx-auto mb-4" />
            <p className="text-fdp-text-3">No notifications yet</p>
          </div>
        ) : (
            ...
            {notifications.map((notification) => (
              <div
                ...
                className={`backdrop-blur-sm rounded-lg border p-4 transition ${
                  notification.read
                    ? 'bg-fdp-surface-1/30 border-fdp-border-2'
                    : 'bg-fdp-surface-1/70 border-blue-500/30'
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-fdp-text-2">{notification.message}</p>
                  </div>
