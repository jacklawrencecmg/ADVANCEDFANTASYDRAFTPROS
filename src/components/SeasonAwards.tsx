...
export function SeasonAwards({ leagueId, season = 2025 }: SeasonAwardsProps) {
  ...
  const getAwardIcon = (awardName: string) => {
    switch (awardName) {
      ...
      default:
        return <Award className="w-8 h-8 text-fdp-text-3" />;
    }
  };
  ...
  const getAwardColor = (awardName: string) => {
    switch (awardName) {
      ...
      default:
        return 'from-fdp-surface-1 to-fdp-surface-2 border-fdp-border-1';
    }
  };
  ...
  if (loading) {
    return (
      <div className="animate-pulse space-y-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="h-32 bg-fdp-border-1 rounded-xl"></div>
        ))}
      </div>
    );
  }
  ...
  if (awards.length === 0) {
    return (
      <div className="text-center py-12 bg-fdp-surface-1 rounded-xl border border-fdp-border-1">
        <Trophy className="w-16 h-16 text-fdp-text-3 mx-auto mb-4" />
        <h3 className="text-lg font-semibold text-fdp-text-1 mb-2">No Awards Yet</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-2xl font-bold text-fdp-text-1">{season} Season Awards</h2>
      </div>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xl font-semibold text-fdp-text-1 mb-2">
                  {award.team_name || 'Unknown Team'}
                </p>
                <p className="text-sm text-fdp-text-1 mb-3">
                  {award.details}
                </p>
                {award.stats && Object.keys(award.stats).length > 0 && (
                  <div className="flex flex-wrap gap-3 text-xs">
                    {Object.entries(award.stats).map(([key, value]) => (
                      <div
                        key={key}
                        className="px-2 py-1 bg-fdp-surface-1/60 rounded-full font-medium"
                      >
                        <span className="text-fdp-text-2">{formatStatKey(key)}:</span>{' '}
                        <span className="text-fdp-text-1">{formatStatValue(value)}</span>
                      </div>
                    ))}
                  </div>
                )}
